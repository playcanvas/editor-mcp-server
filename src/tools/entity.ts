    ...
    server.registerTool(
        'create_entities',
        {
            description: [
                ...
                'Physics note: rigidbody/collision components only simulate if the project has the Ammo WASM module asset imported (Editor: click IMPORT AMMO). Without it, rigidbodies are created but stay frozen at launch — import Ammo before relying on physics rather than pausing to ask. The legacy use3dPhysics/useLegacyAmmoPhysics project settings are not supported and are rejected.',
    ...
    server.registerTool(
        'add_components',
        {
            description: [
                ...
                'Physics note: rigidbody/collision components only simulate when the project has the Ammo WASM module asset imported (Editor: IMPORT AMMO). If it is missing, add the components then import Ammo — do not stall on the decision. The legacy use3dPhysics/useLegacyAmmoPhysics project settings are not supported and are rejected.',
