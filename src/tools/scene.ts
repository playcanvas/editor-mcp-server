...
const Vector3Schema = z.tuple([z.number(), z.number(), z.number()]).describe('An array of 3 numbers');

const SceneSettingsSchema = z.object({
    physics: z.object({
        gravity: Vector3Schema.default([0, -9.8, 0]).describe('An array of 3 numbers that represents the gravity force.')
    }).describe('Physics related settings for the scene.'),
    render: z.object({
        fog: z.string().default('none').describe('The type of fog used in the scene. Can be one of `pc.FOG_NONE`, `pc.FOG_LINEAR`, `pc.FOG_EXP`, `pc.FOG_EXP2`.'),
        ...
        fog_color: Vector3Schema.default([0.0, 0.0, 0.0]).describe('An array of 3 numbers representing the color of the fog.'),
        global_ambient: Vector3Schema.default([0.2, 0.2, 0.2]).describe('An array of 3 numbers representing the color of the scene\'s ambient light.'), // Replace with DEFAULT_AMBIENT_COLOR if needed
        ...
        skyType: z.string().default('infinite').describe('Type of sky.'),
        skyMeshPosition: Vector3Schema.default([0.0, 0.0, 0.0]).describe('An array of 3 numbers representing the position of the sky mesh.'),
        skyMeshRotation: Vector3Schema.default([0.0, 0.0, 0.0]).describe('An array of 3 numbers representing the rotation of the sky mesh.'),
        skyMeshScale: Vector3Schema.default([100.0, 100.0, 100.0]).describe('An array of 3 numbers representing the scale of the sky mesh.'),
        skyCenter: Vector3Schema.default([0.0, 0.1, 0.0]).describe('An array of 3 numbers representing the center of the sky mesh.'),
        skyboxIntensity: z.number().default(1).describe('Multiplier for skybox intensity.'),
        skyboxMip: z.number().default(0).describe('The mip level of the skybox to be displayed. Only valid for prefiltered cubemap skyboxes.'),
        skyboxRotation: Vector3Schema.default([0, 0, 0]).describe('An array of 3 numbers representing the rotation of the skybox.'),
        ...
        lightingCells: Vector3Schema.default([10, 3, 10]).describe('Number of cells along each world-space axis the space containing lights is subdivided into.'),
