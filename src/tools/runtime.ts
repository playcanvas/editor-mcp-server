...
import { LNA_LAUNCH_HINT } from '../wss.ts';
    ...
    server.registerTool(
        'launch_start',
        {
            description: [
                ...
                'This is the prerequisite for all runtime tools. If ready=false, the page may still be loading, popups were blocked, or the browser is blocking its local connection; poll read_runtime_logs or retry.',
        ...
        async ({ waitMs, ...options }) => {
            try {
                ...
                return wss.ok(
                    'launch:start',
                    data,
                    ready ? undefined : { hint: `Runtime did not connect in time. The launch page may still be loading (poll read_runtime_logs). ${LNA_LAUNCH_HINT}` }
                );
            } catch (err) {
